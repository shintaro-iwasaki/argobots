...
ABTI_thread_htable *ABTI_thread_htable_create(uint32_t num_rows)
{
    ...
    p_htable->protected_num_rows = num_rows;
    p_htable->protected_queue = (ABTI_thread_queue *)ABTU_memalign(64, q_size);
    memset(p_htable->protected_queue, 0, q_size);
...
#endif
    ABTU_free(p_htable->protected_queue);
    ABTU_free(p_htable);
}

void ABTI_thread_htablex_push(ABTI_thread_htable *p_htable, int idx,
                             ABTI_thread *p_thread)
{
    ABTI_thread_queue *p_queue;

    if (idx >= p_htable->protected_num_rows) {
        ABTI_ASSERT(0);
        /* Increase the hash table */
        uint32_t new_size;
        new_size = (idx / p_htable->protected_num_rows + 1) * p_htable->protected_num_rows;
        p_htable->protected_queue = (ABTI_thread_queue *)ABTU_realloc(
                p_htable->protected_queue, new_size * sizeof(ABTI_thread_queue));
        memset(&p_htable->protected_queue[p_htable->protected_num_rows], 0,
               (new_size - p_htable->protected_num_rows) * sizeof(ABTI_thread_queue));
        p_htable->protected_num_rows = new_size;
    }

    /* Add p_thread to the end of the idx-th row */
    p_queue = &p_htable->protected_queue[idx];
...
ABT_bool ABTI_thread_htablex_add(ABTI_thread_htable *p_htable, int idx,
                                ABTI_thread *p_thread)
{
    ABTI_thread_queue *p_queue;
...
void ABTI_thread_htablex_push_low(ABTI_thread_htable *p_htable, int idx,
                                 ABTI_thread *p_thread)
{
    ABTI_thread_queue *p_queue;
...
ABT_bool ABTI_thread_htablex_add_low(ABTI_thread_htable *p_htable, int idx,
                                    ABTI_thread *p_thread)
{
    ABTI_thread_queue *p_queue;
