...
#define PREP_REAL_FUNC(func_name)                                              \
    ...
    if (!real_##func_name) {                                                   \
        ...
        func_conv.ptr = dlvsym(RTLD_NEXT, #func_name, dlvsym_ver_##func_name); \
...
static void rtrace_res_finalize(void)
{
...
static void rtrace_op_add(RTRACE_OP_KIND op_kind, size_t val, int success)
{
    ...
    p_op->val = val;
    if (!g_rtrace_global.may_fail && success == RTRACE_SUCCESS) {
...
int rtrace_stop(void)
{
    ...
    /* Check the history and see if there's success.  If there's no success, all
    ...
    if (!p_last_success) {
        ...
        while (p_cur) {
            if (p_cur == p_last_success) {
                ...
                /* Following operations should be successful, so we do not need
                 * its chain. */
...
{
    g_rtrace_global.may_fail = enabled;
}
