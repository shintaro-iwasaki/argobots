...
handover:
    ...
    ABTI_tool_event_thread_yield(p_thread, p_local_xstream,
                                 ABT_SYNC_EVENT_TYPE_MUTEX, (void *)p_mutex);
    ABTI_tool_event_thread_resume(p_next, *pp_local_xstream);
    ABTI_thread *p_prev =
        ABTI_thread_context_switch_to_sibling(pp_local_xstream, p_thread,
                                              p_next);
    /* Invoke an event of thread resume and run. */
    ABTI_tool_event_thread_run(p_thread, *pp_local_xstream, &p_prev->unit_def);
#endif
