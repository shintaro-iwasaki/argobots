...
/**
 * @ingroup MUTEX
 * @brief   Lock a mutex in a blocking form.
 *
 ...
 * already been locked, the caller is blocked on \c mutex until \c mutex becomes
 * available.
 *
 * If \c mutex is recursive, the same caller can acquire multiple levels of
 ...
 * even if the caller is a ULT.  This blocking behavior is deadlock-prone, so
 * the user must carefully use this routine.
 *
 * @contexts
 * \DOC_CONTEXT_ANY \DOC_CONTEXT_NOCTXSWITCH
 *
