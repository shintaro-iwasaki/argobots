 ...
 *
 * @contexts
 * \DOC_CONTEXT_INIT_TASK \DOC_CONTEXT_NOCTXSWITCH
 *
 * @errors
 * \DOC_ERROR_SUCCESS
 * \DOC_ERROR_INV_THREAD_HANDLE{\c thread}
 ...
 * \c thread.  If \c cb_func is \c NULL, \c thread does not call a callback
 * function on its migration.
 *
 * If the callback function is set, a callback function \c cb_func() will be
 * called every time when the associated work unit is migrated.  The first
