...
/*
 * Declare an atomic tagged pointer here.  This atomic type supports very
 * limited functions, which are basically for lock-free stack.
 *
 * If ABTD_ATOMIC_SUPPORT_TAGGED_PTR is not defined, this functionality is not
 * available.
 */
...
/* The following loads follow relaxed/acquire/release semantics, but the
 * operations are not atomic as ABTD_atomic_tagged_ptr (i.e., ptr and tag are
 * accessed in a non-atomic manner). */
...
#else
    *p_ptr = *(void *volatile *)&ptr->val;
    *p_tag = *(size_t * volatile *)&ptr->tag;
#endif
}
...
#else
    *(void *volatile *)&ptr->ptr = ptr;
    *(size_t * volatile *)&ptr->tag = tag;
#endif
}
