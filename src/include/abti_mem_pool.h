...
static inline void *ABTI_mem_pool_alloc(ABTI_mem_pool_local_pool *p_local_pool)
{
    ...
    if (num_headers_in_cur_bucket == 1) {
        ...
        if (bucket_index == 0) {
            ...
            for (int i = 0; i < ABT_MEM_POOL_NUM_TAKE_BUCKETS; i++) {
                                      ...
                                      void *mem)
{
    ...
    if (cur_bucket->bucket_info.num_headers ==
        ...
        if (++bucket_index == ABT_MEM_POOL_MAX_LOCAL_BUCKETS) {
            /* All buckets are full, so let's return some buckets. */
            for (int i = 0; i < ABT_MEM_POOL_NUM_RETURN_BUCKETS; i++) {
                ABTI_mem_pool_return_bucket(
                    p_local_pool->p_global_pool,
                    p_local_pool
                        ->buckets[ABT_MEM_POOL_MAX_LOCAL_BUCKETS - i - 1]);
            }
