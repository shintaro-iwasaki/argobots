 ...
 *
 * If \c cb_func is not \c NULL, the callback function \c cb_func() will be
 * called before all the compartments are set by \c ABT_future_set().  The
 * caller of \c cb_func() is undefined, so a program that relies on the caller
 * of \c cb_func() is non-conforming.  Calling any routine for the future that
 * invokes \c cb_func() in \c cb_func() causes undefined behavior.  The argument
 * \c arg of \c cb_func() is a properly aligned array each of which element
 * stores \c value passed to \c ABT_future_set().  The contents of \c arg are
 * read-only and may not be accessed after \c cb_func() finishes.
 *
 ...
 * \c future will get ready if \c ABT_future_set() for \c future succeeds as
 * many times as the number of its compartments.
 *
 * @note
