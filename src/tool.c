 ...
 * have the corresponding bits.  The other events are disabled.  The user can
 * stop a event callback by setting \c cb_func to \c NULL.
 *
 ...
 * This routine atomically sets \c cb_func(), \c event_mask, and \c user_arg at
 * the same time.
 *
 * @note
 ...
 * the previous \c cb_func() and \c user_arg get unused.  Hence, the user needs to
 ...
 * work unit running on the same execution stream when \c event is \c THRAED_RUN.
 *
