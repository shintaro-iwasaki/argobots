...
static inline void ABTD_thread_terminate(ABTI_thread *p_thread)
{
    ...
    /* Now, the ULT has finished its job. Terminate the ULT. */
    if (ABTD_atomic_load_uint32((uint32_t *)&p_thread->request)
        & ABTI_THREAD_REQ_JOIN_MANY) {
        ...
        if (p_jarg->counter < p_jarg->num_threads) {
            ...
            for (i = p_jarg->counter; i < p_jarg->num_threads; i++) {
                ...
                if (ABTD_atomic_load_uint32((uint32_t *)&p_next->state)
    ...
    /* Since we use atomic_fetch_or to strictly check if waiting threads exist,
     * "acquire" load is not needed. */
    if (p_fctx->p_link) {
