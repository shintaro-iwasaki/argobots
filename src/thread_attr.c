 ...
 * (in bytes) from the ULT attribute \c attr and return values through
 ...
 *
 * If the callback function is set, \c cb_func() will be called every time when
 * the corresponding work unit is migrated.  The first argument of \c cb_func()
 * is the handle of a migrated work unit.  The second argument is \c cb_arg
 * passed to this routine.  The caller of the callback function is undefined, so
 * a program that relies on the caller of \c cb_func() is non-conforming.
 *
 * @contexts
 * \DOC_CONTEXT_INIT \DOC_CONTEXT_NOCTXSWITCH
 *
